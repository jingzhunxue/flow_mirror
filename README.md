...
基于这些自监督预训练的编解码器，我们使用 qwen2 系列 LLM 模型作为初始化参数，整体机构设计如图我们采用的非对称的结构。输入以 Semantic Unit 为主，输出为 Acoustic Unit 和文本同时输出。
![整体结构图](assets/flow_mirror_s_v02_architecture.png)

FlowMirror-s v0.1 和 v0.2 使用了 2 万小时和 5 万小时的语音数据进行端到端的预训练，并支持 ASR、TTS、语音续写、语音对话等任务。这些实验结果初步验证了语音端到端模型的可行性，并且显示出网络设计的可扩展性，预示着模型在后续版本中能够获得更强的能力。
...
### 多任务评估
当前的 checkpoint，经过多任务训练的模型的ASR任务能力，大约相当于 Whisper-small 的水平。公开领域网上语音数据是未训练的数据。Wenet 数据全部未参与端到端训练过程。两部分数据随机采用1024条评估。 
| 数据集来源          | 数量    |  中文CER/WER     |
|-------------------|---------|---------|
